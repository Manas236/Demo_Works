...
- **12 September 2026 — four changes, six commits, pushed.** `62ea74c` the §0
  ...
  `auth.HIDDEN_BLUEPRINTS` — hidden → the spec library through the new leaf
  `specpick.py`, emptied → the product picker **byte-identical to `1d7725a`**
  (a golden captured from that commit's own code, matching); `4086f82`
  **change 2**: SUPPLY ONLY — no installation leg on a quotation, §7 gap 37
  closed; `367dc81` **change 3**: §7 gap 38 closed — an order of extra lines
  alone passes `purchase._parse_lines()` always, and the item rows read the
  switch; `f89dff3` **change 4**: `approval.LADDER_ON = False`, read through
  `ladder_on()`, every reader following it, the measurement cap still binding
  on a saved sheet, every record raised while off stamped at create, the
  access matrix marking the `*.approve` grants `⊗`. ABOUT.md §2g, §2i, §2j,
  §5 `/quotation` and `/purchase`, §7 gaps 37 and 38. **The exact lines to
  flip:** `LADDER_ON = False` in `approval.py`; `HIDDEN_BLUEPRINTS =
  {"product"}` in `auth.py`.

*(The line this replaces read: 11 September 2026, the catalogue is HIDDEN and
the quotation is written from the SPEC LIBRARY — the owner's own decision; not
a CC-2 item, no charge, and the board does not move.)*
...
named the **ELEVENTH** pass of 30 August 2026 and the board as **17 of 20** —
both stale. **The TWELFTH, THIRTEENTH and FOURTEENTH passes left no entry
here at all**, and this pass has not written their entries for them: an entry
reconstructed afterwards by somebody who did not do the work is worse than a
visible gap. `PROGRESS.md` §4c is complete for all of them (rows 25 through 28)
and `CLIENT_CHANGES.md` §0 carries a dated block for each; **those two are the
record, and this file is the index that fell behind.**
...
- **FIFTEENTH pass, 6 September 2026 — BEYOND CC-2, and no bar moved.**
  ...
  block. **`tools/e2e_chain.py`** is a standalone end-to-end HTTP chain driver
  — an empty project through to a fully claimed BOQ, over the wire, with twelve
  tally assertions and a reverse-dependency-order teardown. It exists because
  the suite runs with `DB_ENABLED=false` and a cleared `STORE`, so 2,081 green
  ...
  unit-bearing numeric columns under spanning `SUPPORTS` and `SPRINKLER` heads,
  seventeen seeded locations, a TOTAL row over **every** column, and a
  **two-party countersignature** — the reason the document is called *joint*.
  ⚠⚠ **The brief instructed that measurement stop capping the RA claim; both
  the cap and the per-line `line_id` link already existed and shipped, so that
  would have deleted two live guards silently and contradicted CC-2's C2.
...
reports **2,791 passed / 4 skipped** against the same commit, and read
...
**2,661 / 4** at the start of the second 14 September change (measured, matching) and
**2,655 / 4** at the start of the first 14 September change (measured, matching) and
**2,651 / 4** at the start of the 13 September change (measured, matching) and
**2,433 / 4** at the start of the 12 September pass (measured, matching) and
