...
| | |
|---|---|
| **Date** | 9 September 2026, **SEVENTEENTH pass** *(a **DEFECT** pass. It moves **no bar** and builds **no CC-2 item**. Two items: ABOUT.md §7 gap 33, the BOQ→project linkage, **reclassified from new scope to a defect** because MG/SF/2026-02 §3 lists **F.04 — “Project grouping — BOQs grouped under a project”** among the five items stated to the client as built and handed over at no charge; and a **float-comparison audit** on the over-claim and measurement ceilings which **changed no code** and is recorded as §7 gap 34 with its measured residuals.)* |
| **Machine** | home laptop &mdash; `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `b009749` — *“Float audit on the over-claim and measurement ceilings: NEGATIVE result, no code changed”*, the fourth of the five commits of this pass — **in the order the brief required**: `e5547e6` the §0 block **alone and first**, `79b0d34` gap 33 (A1/A2/A3 and its tests), `f32fae0` the driver's assertion 8, `b009749` the float audit, and this edition fifth. One commit per item, as required. |
| **vs `origin/antigravity-dev`** | **measured at the start of this pass with `git rev-list --left-right --count`: 5 ahead, 0 behind**, tip `f77919d`. That matches what the brief recorded as last known, and it was **confirmed by measurement rather than quoted**. **Nothing was pushed.** |
| **Dirty files** | **0. `git status --porcelain` was empty at the start of this pass**, and the branch was verified as `antigravity-dev` before anything was written. One backup **PAIR** was taken before any write — `backups/samruddhi_qms-20260909-103122-pre-pass17.sql` (**685,533 bytes**) and `…-pre-pass17-attachments.zip` (**22 bytes, 0 files** — the store is empty and the zip is written anyway, which is `tools/backup_db.py`'s own rule). A second pair was taken **after** the live driver run, `…-20260909-105810-post-pass17.sql` (**685,533 bytes**), for the closing diff. <br><br>⚠ **FOUR dev-app processes were found running and were stopped before anything touched the database** — PIDs **5844** and **23792** left by the sixteenth pass (started 21:37), plus **16600** and **24340**, reloader children spawned at 00:13:46. PID 5844 held `127.0.0.1:5000`. They were **hard-killed deliberately**: a graceful stop risks a stale in-memory STORE being written back over the previous pass's `grid_model` marks. |
| **Test figure** | **measured at the end of the pass: 2,283 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **2,284 passed, 4 skipped** against this commit. **Both were measured in this pass, neither was quoted.** The baselines were **re-measured at the start of this pass rather than taken from the brief** — **2,235 / 2** and **2,236 / 4** — and both matched what the sixteenth pass recorded. <br><br>**+48 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file**: `tests/test_boq_project_link.py` (**17**, new) and `tests/test_qty_float_precision.py` (**31**, new). 17 + 31 = **48**. The row-2/row-3 relationship holds unchanged: 2,283 + 1 passed and + 2 skipped = 2,284 / 4. <br><br>⚠ **NO GOLDEN MOVED.** All sixteen digests in `tests/test_print_golden.py` pass unchanged; `/boq/create` is not golden-pinned and neither is `/projects/view/<id>`. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE** — **no route was added and no permission was minted.** Detach is a new *action* on the **existing** `projectview.view_project` endpoint, which is already in `auth.ROUTE_PERMISSIONS` as `project.view` with a per-view `project.edit` guard on its POST branch, so there was nothing to classify and nothing to regenerate. `tools/reconcile_role_permissions.py` was run anyway: **7 roles, 79 permissions, no drift, nothing to do.** <br><br>⚠ **One existing test was REWRITTEN and NOT weakened**, keeping its old assertion **verbatim** in a comment above it: `tests/test_privilege_escalation.py::test_the_refusal_reaches_the_access_log` asserted `len(auth.REFUSAL_LOG) > before`. That deque is `maxlen=500` and the suite now saturates it — **measured at exactly 500** — so `appendleft` evicts from the tail and the length can never grow. The assertion was first unfalsifiable and then false, and this pass's one extra refusal is what tipped it. The rewrite asserts the **newest entry IS this refusal**, which is strictly stronger and does not saturate. <br><br>⚠ **Every guard touched is mutation-proved and the proofs were RUN. Fourteen mutations, all fourteen caught.** Eight on gap 33 (control removed, preselection dropped, POST not storing, revision reading the form, detach not walking the chain, detach ignoring which project a member is on, the `project.edit` guard disabled, attach not walking the chain), five on the float guards (rounding removed from `ra.py`, rounding removed from `measurement.py`, the `ra.py` guard disabled, `OVERCLAIM_TOLERANCE` raised to 0.01, the two epsilons made to disagree), and one on the rewritten refusal-log assertion (`REFUSAL_LOG.appendleft` stubbed out). |
