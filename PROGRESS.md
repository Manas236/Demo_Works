...
| | |
|---|---|
| **Date** | 11 September 2026, **the pass after the seventeenth** *(⚠ **the ordinal is not assigned**: the 10 September commit `1d7725a` — BOQ insert no longer scrolls, orphaned-project BOQs shown, two layout fixes — recorded no pass number here and moved no figure in any document; its 61 tests are absorbed into this pass's measured baseline rather than back-attributed. This pass is the **owner's own decision**, CLIENT_CHANGES.md §0 twenty-seventh block: the **product catalogue is HIDDEN from everybody** behind `auth.HIDDEN_BLUEPRINTS`, and **a quotation is written from the SPEC LIBRARY** under a narrow unfreeze of three `quotation.py` functions. **Not a CC-2 item, no charge, moves no bar.**)* |
| **Machine** | home laptop &mdash; `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `ad3ec8d` — *“Part 2: a new quotation is written from the SPEC LIBRARY — quotation.py unfrozen narrowly for three functions”*, the third of the four commits of this pass, **in the order the brief required**: `e56f679` the §0 block **alone and first**, `10884ba` Part 1 (the hide: `auth.HIDDEN_BLUEPRINTS`, the roles editor, the access matrix, `tests/test_product_hidden.py`), `ad3ec8d` Part 2 (the picker, `tests/test_quotation_spec_picker.py`, the freeze test rewritten), and this edition fourth. One commit per step, suite green before each. |
| **vs `origin/antigravity-dev`** | **measured at the end of this pass with `git rev-list --left-right --count`: 3 ahead, 0 behind** before the documents commit, tip `1d7725a` — which is **not** what the seventeenth pass recorded (5 ahead of `f77919d`): the branch was pushed between the two passes by somebody other than an agent, and this pass records that rather than reasoning about it. **Nothing was pushed by this pass.** |
| **Dirty files** | **0. `git status --porcelain` was empty at the start of this pass**, and the branch was verified as `antigravity-dev` at `1d7725a` before anything was written; `Quote.html`, recorded as a known untracked file, was **not present**. **Four dev-app processes were found running and hard-killed** before the backup — PIDs 4060, 24168, 2932, 8456, two on the `.venv` interpreter and two on the global one. One backup **PAIR** was taken before any write — `backups/samruddhi_qms-20260911-102840-pre-hide-product-spec-quotation.sql` (**685,533 bytes**, byte-for-byte the seventeenth pass's size: the data has not moved) and `…-attachments.zip` (**22 bytes, 0 files**). **The app was not started and nothing in this pass writes to the database.** |
| **Test figure** | **measured at the end of the pass: 2,432 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **2,433 passed, 4 skipped** against this commit. **Both were measured in this pass, neither was quoted.** ⚠ **The baselines were re-measured at the start of this pass and DID NOT match the documents**: **2,344 / 2** and **2,345 / 4**, against the recorded **2,283 / 2** and **2,284 / 4** — 61 above in both, because the 10 September commit `1d7725a` added `tests/test_editor_nav.py` and moved no figure anywhere. The pass held itself to the **measured** baseline. <br><br>**+88 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file**: `tests/test_product_hidden.py` (**40**, new — hidden → 403 for all seven roles on all four routes and both verbs, nothing destroyed, no card, link or count, the roles-editor round trip, the `⊘` mark; un-hidden → today's behaviour) and `tests/test_quotation_spec_picker.py` (**48**, new — the mapping, the empty-price rule, the one-tax refusal, identity-from-the-library, and **ten cases running the page's real JavaScript under Node** through the rendered controls; they skip without Node). 40 + 48 = **88**. Existing product-page tests run under the new `catalogue_unhidden` fixture and were **not** rewritten into 403 checks; the freeze test in `tests/test_nav_user_chip.py` was **rewritten and not weakened**, old assertion kept verbatim, to hold the diff to the four named functions — and its git-diff helper now decodes UTF-8, because `text=True` is cp1252 on Windows and a `═` in the new diff was returning `stdout=None`. The row-2/row-3 relationship holds unchanged: 2,432 + 1 passed and + 2 skipped = 2,433 / 4. <br><br>⚠ **NO GOLDEN MOVED.** All sixteen digests in `tests/test_print_golden.py` pass unchanged; the quotation's printed sheet, the PI and the TI were not touched. <br><br>⚠ **`docs/ACCESS_MATRIX.md` MOVED and was regenerated** — no permission was minted and no route was added (**7 roles · 79 permissions · 127 classified endpoints**, unchanged), but the three `product.*` rows now carry the new **`⊘` module-hidden** mark in place of `·`, and the appendix names the four hidden endpoints. `tools/dump_access_matrix.py` grew the mark. <br><br>⚠ **Eleven mutations, eleven caught, and the proofs were RUN.** Five on the hide (the gate's hidden branch disabled, `can_reach()`'s mirror disabled, hidden grants dropped on a role save, the dashboard metrics ignoring the toggle, the roles editor drawing the boxes live) and six on the picker (the one-tax check disabled, an empty price accepted on a shown line, identity trusted from the form, a gone spec dropped silently, the SAC not used on the installation leg — and an edit to `_next_ref()`, OUTSIDE the four unfrozen functions, which the rewritten freeze test caught). <br><br>⚠ **One item of the brief was NOT done, as the brief itself required:** `/purchase/create`'s catalogue rows still read the catalogue. `purchase._parse_lines()` refuses an order with no catalogue item and a free-text extra line does not satisfy it, so switching the rows off would have made a from-scratch purchase order impossible to raise — the brief said to stop and report in exactly that case. ABOUT.md §7 gap 38. <br><br>**GST rates in the seeded library, counted as the brief asked:** `supply_gst_rate` — one distinct value, 18.0, on all 56 clauses; `install_gst_rate` — one distinct value, 18.0, on all 56; **0 clauses whose two legs differ.** The one-tax guard cannot fire on the seed and exists for the library as it is edited. |
