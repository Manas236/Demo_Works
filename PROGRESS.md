...
| **HEAD** | `a06f6ea` — *"Close the login timing oracle that enumerated valid usernames"*. This file is updated in the docs commit that follows it, so the code it describes is the code at `a06f6ea`. |
| **vs `origin/antigravity-dev`** | **4 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed. The four are the escaping fix, the adversarial test file, and one commit per hole it found. The previous edition's count restarted from the base pushed at `2286c14`; that base has since moved to `005e3ec`, which is what these four sit on. |
| **Dirty files** | 0 — `git status --porcelain` empty at the time of writing. |
| **Test figure** | **measured this pass: 1,058 passed, 1 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **986 passed / 1 skipped**, re-measured in the same configuration at the start of the pass rather than quoted — it matched. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **1,059 passed, 3 skipped** against this commit and **987 / 3** against the pre-pass code, both measured. The third configuration (*openpyxl present, workbooks present*) is still unmeasured — neither client workbook is on this box. <br><br>+72: `tests/test_escaping.py` (31), `tests/test_access_control_adversarial.py` (29), `tests/test_access_matrix_doc.py` (12). ⚠ **Five print goldens were re-baselined**, +8 bytes each in two blocks, with the byte-level justification recorded beside the digests — the `&` in the company tagline now prints `&amp;`. No rendered figure moved. |
