...
| | |
|---|---|
| **Date** | 6 September 2026, **FIFTEENTH pass** *(a pass that again moves **no bar at all**. **Not one of CC-2's twenty items is touched.** Two things were built — `tools/e2e_chain.py`, a standalone end-to-end HTTP chain driver, and the **JOINT MEASUREMENT SHEET**, a redesign of C2's printed document onto the client's own workbook layout — authorised by a single new `CLIENT_CHANGES.md` §0 block, the **first of 6 September 2026** and the **twenty-third occasion overall**. They go to §4c as **rows 29 and 30**, outside the bars. ⚠ **The brief for this pass carried FOUR wrong premises and the first of them would have deleted two shipped guards** — see the Header note below and the §0 block; all four are corrected there rather than propagated. **C2 was BUILT before this pass and is BUILT after it: redrawing a document's printed sheet does not move an item.** 3A stays 6 of 6, 3B stays 8 of 8, 3C stays 5 of 6. ⚠ **C6 is STILL BLOCKED on Open question 4 and nothing here touches it.**)* |
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `af50ae5` — *"Measurement: the JOINT MEASUREMENT SHEET — a location × diameter grid, countersigned"*, the third of the four commits of this pass — **in the order the brief required, and the order is the point**: `49a06cb` the §0 block **alone**, `9608fb5` the driver and its 63 offline tests, `af50ae5` the measurement sheet (carrying its own ABOUT.md data-shape edit, as the brief required), and this edition fourth. ⚠ **The driver was written and its helpers pinned BEFORE the measurement work began**, deliberately, so that it is a regression net rather than a claim co-delivered with the thing it is supposed to check. |
| **vs `origin/antigravity-dev`** | **84 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed, and measured **before this edition was itself committed**, which adds one more. The pass began at **81 ahead, 0 behind**, which matched the brief's own estimate exactly. ⚠ **`main` is separately 21 ahead of `origin/main`** and this pass did not touch it. **Nothing has been pushed in a long while and that is not this pass's to decide.** |
| **Dirty files** | **0. `git status --porcelain` was empty at the start of this pass.** One backup **PAIR** was taken before any work — `backups/samruddhi_qms-20260906-183630-pre-pass-h.sql` (**685,034 bytes**) and its `-attachments.zip` (**22 bytes, 0 files**). ⚠⚠ **NO LIVE RECORD WAS MUTATED BY THIS PASS, and that is a weaker statement than it looks** — it is true because the driver **was never run against the live database**, not because it was run and cleaned up. See the Header note. `tools/backfill_measurement_grid.py` ships in this pass and **has not been run**; the two live sheets are unmarked and render legacy through the absence of a mark, which is correct but is not the same as having been migrated. ⚠ The live database was **read** in this pass and written **never**. ⚠ The scratch space used was the session temp directory, outside the repository tree, and nothing committed references it. |
| **Test figure** | **measured at the end of the pass: 2,204 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **2,081 passed / 2 skipped**, re-measured in that configuration at the start of this pass rather than quoted — it matched the previous edition and the brief exactly. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **2,205 passed, 4 skipped** against this commit and **2,082 / 4** at the start of the pass — both measured, neither derived, and both matched. The third configuration (*openpyxl present, workbooks present*) is still unmeasured; neither client workbook is on this box. <br><br>**+123 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file rather than attributed**: `tests/test_e2e_chain_helpers.py` (**63**), `tests/test_measurement_joint.py` (**42**), `tests/test_measurement_grid_backfill.py` (**13**), `tests/test_print_golden.py` (**+5**, 11 → 16) and `tests/test_import_directions.py` (**net 0** — one new test, and one parametrised `FORBIDDEN` row removed). 63 + 42 + 13 + 5 + 0 = **123**. The row-2/row-3 relationship holds unchanged: 2,204 + 1 passed and + 2 skipped = 2,205 / 4. <br><br>⚠ **NO EXISTING GOLDEN MOVED.** All eleven recorded digests in `tests/test_print_golden.py` pass unchanged. The five additions are **new coverage, not a re-baseline** — the measurement sheet has never had a golden, deliberately, and the joint sheet earns one because its layout is **transcribed from the client's own workbook** rather than a design nobody specified. `measurement.print_ms` joins `dashboard.PINNED_PAGES`, which is how `test_nav_user_chip.py`'s cross-check said it was needed rather than how anybody remembered. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE, and was regenerated rather than assumed** — `python tools/dump_access_matrix.py` produces a byte-identical file. **7 roles · 79 permissions · 127 classified endpoints**, all three unchanged: **no route was added and no permission was minted**. `tools/reconcile_role_permissions.py` was run anyway and reported **nothing to do**. <br><br>⚠ **Two existing tests were rewritten and NEITHER was weakened or deleted**, each keeping its old assertion **verbatim** in a comment. (1) `test_measurement.py::test_the_printed_sheet_reuses_the_shared_document_furniture` pinned `.items-wrap` and `.sig-block`; the joint sheet carries neither, because `docsheet.items_table()` builds a ONE-row head and the sheet's is two rows deep with spanning group cells, and because the foot is **two parties side by side** rather than one signatory — which is the whole reason the document is called *joint*. Everything shareable is still shared and is still asserted. (2) `test_import_directions.py`'s `measurement → settings` ban, **whose stated premise stopped being true**: it read *"the measurement series is FY-scoped … and owns no /settings counter"*, and measurement now owns a /settings **record** (the twelve column definitions). Replaced by a test that **walks the import graph and proves no cycle**, and names the `challan.py` / `po_draft.py` precedent. <br><br>⚠ **Every new guard is mutation-proved, and the proofs were RUN. Fifteen mutations, fifteen caught.** Seven on the driver's helpers (teardown order, override-append, banker's rounding, per-line GST, `money_after` returning 0 instead of `None`, a `KNOWN-BAD` that can never fail, an unknown teardown kind silently ignored) and seven on the measurement guards — **the largest being a joint sheet that drops its `items` rows, which fails EIGHT tests**, and that is the one that matters: it is the shape the brief asked for. Plus one on the escaping sweep, below. <br><br>⚠⚠ **THREE OF THIS PASS'S OWN NEW TESTS WERE VACUOUS WHEN WRITTEN, AND ONLY MUTATION FOUND THEM.** (1) The per-slab GST test used an input where per-line and per-slab rounding **agree**, so it asserted nothing; it now uses two rows of ₹2.75 at 18%, where the two schemes give 100 and 99 paise. (2) A `text_of` expectation was simply stale. (3) **The worst: `tests/test_escaping.py`'s poison sweep did not reach the joint sheet at all.** `/measurement/print` renders `ms-2`, the only approved fixture, and `ms-2` was legacy-shaped — so with all four new sinks emitting **raw**, the sweep stayed green at 33 passed. **Measured, not assumed.** `ms-2` is now the joint model, and the same mutation now fails four tests. That is the third time this repository has found the same failure and the second time in that one file. |
