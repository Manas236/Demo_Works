...
| | |
|---|---|
| **Date** | 5 September 2026, **FOURTEENTH pass** *(a pass that moved **no bar at all**, and that is again the honest headline. **Not one of CC-2's twenty items is touched.** Four visual cues on the **existing** dashboard — authorised by a single new `CLIENT_CHANGES.md` §0 block, the **first of 5 September 2026** and the **twenty-second occasion overall**. It goes to §4c as **row 28**, outside the bars. ⚠ **The brief for this pass cited a *"Visual Dashboard is free, not chargeable"* ruling of 19/24 August 2026 that DOES NOT EXIST in this record**, and cited the board as **18/20** when it was and remains **19/20**; both are corrected in the §0 block rather than propagated. CC-2's untagged "visual dashboard" lines 1 and 2 stay untagged, unpriced and **unanswered**. 3A stays 6 of 6, 3B stays 8 of 8, 3C stays 5 of 6. ⚠ **C6 is STILL BLOCKED on Open question 4 and nothing here touches it.**)* |
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `80e7390` — *"Dashboard: claimed-against-approved bars, per project"*, the fourth of the four commits of this pass — **one per item, as the brief required**: item 1 the two status tiles (`57f7960`), item 2 the claimed-to-date tile (`030683b`), item 3 the permission-filtered activity feed (`ed1ceb2`), item 4 the per-project bars (`80e7390`). This edition is written **after** them and describes the code as they leave it; it is itself the fifth, alongside the §0 block and the ABOUT.md §5 entry. |
| **vs `origin/antigravity-dev`** | **80 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed, and measured **before this edition was itself committed**, which adds one more. The pass began at **76 ahead, 0 behind**, one more than the previous edition predicted because that edition was itself committed after writing the figure. ⚠ **`main` is separately 21 ahead of `origin/main`** and this pass did not touch it. **Nothing has been pushed in a long while and that is not this pass's to decide.** |
| **Dirty files** | **0, and the gate passed cleanly for the first time in four passes.** `git status --porcelain` was **empty** at the start of this pass — the `31AUG.md` scratch note that stopped the twelfth and thirteenth passes is gitignored by class and no longer trips it. ⚠ **NO LIVE RECORD WAS MUTATED BY THIS PASS.** One backup **PAIR** was taken before any work — `backups/samruddhi_qms-20260905-163844-pre-dashboard-cues.sql` (**685,034 bytes**) and its `-attachments.zip` (**22 bytes, 0 files**) — and **no migration, backfill or cleanup was run at all.** The live database was **read** in this pass, repeatedly and deliberately, and written **never**. ⚠ The scratch space used was the session temp directory, outside the repository tree, and nothing committed references it. |
| **Test figure** | **measured at the end of the pass: 2,081 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **2,034 passed / 2 skipped**, re-measured in that configuration at the start of this pass rather than quoted — it matched the previous edition exactly. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **2,082 passed, 4 skipped** against this commit and **2,035 / 4** at the start of the pass — both measured, neither derived, and both matched the previous edition exactly. The third configuration (*openpyxl present, workbooks present*) is still unmeasured; neither client workbook is on this box. <br><br>**+47 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file rather than attributed**: one new file, `tests/test_dashboard_boq_ra.py` (**47**), and **no existing test file changed its count**. The row-2/row-3 relationship holds unchanged: 2,081 + 1 passed and + 2 skipped = 2,082 / 4. <br><br>⚠ **NO GOLDEN MOVED.** All eleven assertions in `tests/test_print_golden.py` pass unchanged, as do `test_page_chrome.py` and `test_nav_reachability.py`. **Nothing in this pass touches a print route** — the dashboard is not one, `_nav()` is not changed, and no module that feeds a golden was edited at all. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE, and was regenerated rather than assumed** — `python tools/dump_access_matrix.py` produces a byte-identical file. **7 roles · 79 permissions · 127 classified endpoints**, all three unchanged: **no route was added and no permission was minted**, so no role needed a grant. `tools/reconcile_role_permissions.py` was run anyway and reported **nothing to do**. <br><br>⚠ **Two existing tests failed and NEITHER was weakened or deleted**, each keeping its old assertion **verbatim** in a comment. (1) `test_nav_visibility.py::test_the_quotation_surfaces_go_together` asserts an Operation Head is shown no `>Register<`; the new panel's header link was labelled *"Register"* and legitimately opens for that role. **Fixed on the code's side, not the test's** — the label now names which register, which is what a page carrying two of them should have said anyway. (2) `test_page_reachability.py`'s computed-endpoint tripwire fired at **19**; two of the three new computed `url_for()` sites were then **rewritten as literal branches** so they contribute real edges, and the bound was raised **16 → 17** for the one that is structurally computed, documented as the two previous raises were. <br><br>⚠ **Every new guard is mutation-proved, and the proofs were RUN rather than asserted. Thirty-two mutations, thirty-two failures**: six on item 1 (the grandfather exclusion, the cancelled exclusion, rejected-as-pending, `_boq_is_open` always true, an emptied superseded set, the cancelled predicate normalising unknown as cancelled), five on item 2, ten on item 3 (**three of them on the permission filter itself** — removed entirely, always-true, and keyed on the wrong endpoint) ten on item 4, and one on the percentage label. <br><br>⚠ **TWO OF THIS PASS'S OWN NEW TESTS WERE VACUOUS WHEN WRITTEN, AND ONLY THE MUTATIONS FOUND THEM.** (1) Swapping `claim_subtotal` for `net_payable` changed nothing observable, because every fixture set the two fields equal; the fixture now withholds a 10% retention and two tests fail on that swap. (2) The activity-feed cap test asserted only the `"+ N more"` label, which is computed from `len(rows)` and stays correct when the cap on the render loop is removed — it now counts drawn rows. **A guard nobody has tried to break is a guard nobody has tested**, and that goes on applying to this pass's own work. |
