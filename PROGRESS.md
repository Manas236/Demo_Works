...
| | |
|---|---|
| **Date** | 30 August 2026, **ELEVENTH pass** *(a **beyond-CC-2** pass, the fourth running. It builds no Phase 3 item and closes none: the bars below do not move and the denominator stays 20. Three things, all recorded in §4c rows 22-24 and in a new `CLIENT_CHANGES.md` §0 block — the **sixth** of that date — and **none is in CC-2**. **A `project_id` ON AN ATTENDANCE MARKING**, picked from a list filtered to the marking's own site, where several projects on that site **require** a choice rather than getting one. **The Site Labour section split into two labelled groups**, booked and unattributed, summed separately and never added together. **And a backfill** that links the unambiguous markings and prints the rest with their candidates. ⚠ **C6 is STILL BLOCKED on Open question 4 and nothing here answers it** — attributing a day is not costing a project.)*
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `4832dc3` — *"The backfill was run live, and the two leftovers are resolved"*, the fifth of the five commits before this one. This edition is written **after** them and describes the code as they leave it; it is itself the sixth. **B8 is still AUTHORISED and UNBUILT** and this pass did not touch it. **C6 is still BLOCKED** and this pass did not unblock it.
| **vs `origin/antigravity-dev`** | **65 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed, and measured **before this edition was itself committed**, which adds one more. The pass began at **60 ahead, 0 behind** — the figure its brief predicted, matched exactly.
| **Dirty files** | 0 — `git status --porcelain` empty at the start of this pass and empty again at the end of it. ⚠ **THIS PASS MUTATED LIVE RECORDS** — 8 attendance markings gained a `project_id`, and 2 projects were **renamed** — under the sixth 30 August 2026 override block, with **three** `mysqldump`s bracketing it: `…-223324-pre-marking-projects.sql` (**460,518 bytes**) before any change; `…-223353-post-backfill-pre-rename.sql` (**461,506**, 988 larger) after `tools/backfill_marking_projects.py --write`; and `…-223423-post-rename.sql` (**461,606**, 100 larger) after the two renames. The tool was dry-run first and re-run afterwards to prove the second run is a no-op. ⚠ **NOTHING WAS CREATED AND NOTHING DELETED** — every collection count is byte-for-byte what it was, which is what a field-only migration should look like. ⚠ **NEITHER LEFTOVER PROJECT WAS DELETED**: *"Test Supplier"* was found to carry a BOQ and two RA bills, so the delete branch did not apply and it was renamed like the other. `backups/` is gitignored and was not staged.
| **Test figure** | **measured at the end of the pass: 1,927 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **1,897 passed / 2 skipped**, re-measured in that same configuration at the start of this pass rather than quoted — it matched what the previous edition recorded, exactly. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **1,928 passed, 4 skipped** against this commit and **1,898 / 4** at the start of the pass — both measured, neither derived, and both matched the previous edition exactly. The third configuration (*openpyxl present, workbooks present*) is still unmeasured; neither client workbook is on this box. <br><br>**+30 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file rather than attributed**: one new file — `tests/test_backfill_marking_projects.py` (**15** — the never-guesses property with its control, the two tie-breaks named so adding either goes red, the ambiguous report's contents, the freeze on an already-attributed marking, one-field-only, dry run, idempotence, and AST checks that the only flag is `--write` and no similarity function exists) — plus **+14** in `tests/test_attendance.py` (44→58) and **+1** parametrised case in `tests/test_import_directions.py` (294→295: `attendance → project`). 15+14+1 = 30. <br><br>⚠ **NO GOLDEN MOVED.** All eleven assertions in `tests/test_print_golden.py` pass unchanged, as do `test_nav_reachability.py` and `test_page_chrome.py`. Nothing in this pass touches a printed sheet or the nav. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE, and that was VERIFIED rather than assumed** — regenerated with `python tools/dump_access_matrix.py` and diffed: no change. **7 roles · 79 permissions · 114 classified endpoints**, all three unchanged. **No permission was minted and no route was added** — the picker renders inside `/attendance/mark` and `/attendance/edit/<id>`, both already classified write endpoints — so `tools/reconcile_role_permissions.py` was run and reported **nothing to do**. <br><br>⚠ **Three existing tests were rewritten and NONE was weakened or deleted**, each keeping its old assertion **verbatim** in a comment above it: `test_the_labour_section_consumes_the_module_and_does_not_reimplement_it` (the accessor list grew; the banned-arithmetic half is **unchanged**, and it now also refuses a return to filtering the raw site list on the page), `test_the_labour_section_says_the_rows_are_not_tagged_to_the_project` (the old wording is now the **unattributed group's** caption, and that is what is asserted — saying it of the whole panel would be the old assertion's own lie pointing the other way), and `test_the_labour_section_names_every_other_project_on_the_same_site` (all three original assertions kept; the fixture gains the unattributed marking the note is about, plus a **second control**: attribute it and the note must vanish while the sibling stays). <br><br>⚠ **Every new guard is mutation-proved and the proofs are TESTS rather than notes:** a project at another site posted at the write (it must not reach the record, and the control is that the right project at the same site does), the required choice where a site carries two (with the control that choosing one saves), the two group totals asserted at 1,000 and 250 with **1,250 appearing nowhere**, the backfill's refusal to tie-break by age in either direction, and the freeze on a marking that already carries a project. |
...
```
ALL  ██████████████████░░  18 of 20 BUILT · 0 PARTIAL · 2 BLOCKED ·  0 NOT STARTED
...
`before_request` gate that refuses **any** of the **120** endpoints not classified in
