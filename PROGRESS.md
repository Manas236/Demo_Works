...
| | |
|---|---|
| **Date** | 27 August 2026 *(the day's second pass — the escaping and adversarial pass is the first, and `a06f6ea` is where it ended)* |
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `a6137a6` — *"Break-glass password recovery, and a second Owner instead of one"*. This file is updated in the docs commit that follows it, so the code it describes is the code at `a6137a6`. |
| **vs `origin/antigravity-dev`** | **6 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed. Five are the escaping fix, the adversarial test file, one commit per hole it found, and that pass's docs commit; the sixth is the break-glass tool. The docs commit carrying this edition makes seven. |
| **Dirty files** | 0 — `git status --porcelain` empty at the time of writing. |
| **Test figure** | **measured this pass: 1,062 passed, 1 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **1,058 passed / 1 skipped**, re-measured in the same configuration at the start of the pass rather than quoted — it matched. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **1,063 passed, 3 skipped** against this commit, and read **1,059 / 3** against the pre-pass code — both measured, neither derived. The third configuration (*openpyxl present, workbooks present*) is still unmeasured — neither client workbook is on this box (`fixtures/` holds only its README). <br><br>+4, all in `tests/test_auth.py`: the four structural invariants on `tools/set_password.py`. Its behaviour needs a live MySQL and cannot run in the suite. <br><br>✅ **No golden was re-baselined and none moved** — no golden file is modified in the working tree and `tests/test_print_golden.py` passes unchanged. `docs/ACCESS_MATRIX.md` needed no regeneration: no permission, role or route classification changed. <br><br>*(The two figures this row replaced, 1,058 / 1 and 1,059 / 3, were the escaping pass's and were correct; ABOUT.md §1's table was the one file that still carried the 26 August numbers, and this pass corrected it.)* |
...
⚠ **Nothing in this section changed build state on 27 August 2026.** That pass
escaped user text everywhere it reaches HTML and **attacked** the layer B1–B5
shipped; it built no Phase 3 item and started nothing gated. B1–B5 stay BUILT,
B6–B8 stay NOT STARTED and gated. What it changed is how much of B1–B5 is
*verified* rather than asserted, and that is recorded in the B1, B5 and B6 rows
below. [STATE.md §1.12](STATE.md) is the plain-English account;
