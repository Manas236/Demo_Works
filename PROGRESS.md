...
| | |
|---|---|
| **Date** | 30 August 2026, **TENTH pass** *(a **beyond-CC-2** pass, the third running. It builds no Phase 3 item and closes none: the bars below do not move and the denominator stays 20. Three things, all recorded in §4c and in a new `CLIENT_CHANGES.md` §0 block — the **fifth** of that date — and **none is in CC-2**. **A SITE LABOUR section on `/projects/view/<id>`**, listing the markings booked at a project's site and saying on its face that they are booked there rather than tagged to the project. **A live data cleanup** that folds one duplicate address and purges five named test records. **And a demo scenario tool** so the joins can be seen working. ⚠ **C6 is STILL BLOCKED on Open question 4 and nothing here answers it** — the section presents markings and is not a P&L authority.)*
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `f9d7414` — *"The demo scenario was seeded on the live database, and the result is measured"*, the sixth of the six commits before this one. This edition is written **after** them and describes the code as they leave it; it is itself the seventh. **B8 is still AUTHORISED and UNBUILT** and this pass did not touch it. **C6 is still BLOCKED** and this pass did not unblock it.
| **vs `origin/antigravity-dev`** | **59 ahead, 0 behind, unpushed** — measured with `git rev-list --left-right --count`, not assumed, and measured **before this edition was itself committed**, which adds one more. The pass began at **53 ahead, 0 behind** — the figure its brief predicted, matched exactly.
| **Dirty files** | 0 — `git status --porcelain` empty at the start of this pass and empty again at the end of it. ⚠ **THIS PASS DELETED LIVE RECORDS AND CREATED OTHERS**, under the fifth 30 August 2026 override block, and **five** `mysqldump`s bracket it: `…-174041-pre-site-labour.sql` (**455,600 bytes**) before any change; `…-181604-pre-clean-write.sql` (**455,600**, identical — nothing between them touched a live record) and `…-181629-post-clean-write.sql` (**451,659**, 3,941 smaller) around `tools/clean_site_data.py --write`; `…-182353-pre-seed-demo.sql` (**451,659**, identical again) and `…-182527-post-seed-demo.sql` (**460,518**, 8,859 larger) around `tools/seed_demo_scenario.py --write`. Both tools were dry-run first and both were re-run afterwards to prove the second run is a no-op. ⚠ **ONE PURGE WAS REFUSED BY THE TOOL AND THE RECORD IS STILL THERE** — see §4c row 20. `backups/` is gitignored and was not staged.
| **Test figure** | **measured at the end of the pass: 1,897 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The baseline it moved from was **1,847 passed / 2 skipped**, re-measured in that same configuration at the start of this pass rather than quoted — it matched what the previous edition recorded, exactly. <br><br>The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **1,898 passed, 4 skipped** against this commit and **1,848 / 4** at the start of the pass — both measured, neither derived, and both matched the previous edition exactly. The third configuration (*openpyxl present, workbooks present*) is still unmeasured; neither client workbook is on this box. <br><br>**+50 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file rather than attributed**: two new files — `tests/test_clean_site_data.py` (**20** — the orphan refusal per dependant collection, the `references_of()` stop, the skip, the dry run, and AST checks that the only flag is `--write` and no similarity function exists) and `tests/test_seed_demo_scenario.py` (**16** — unreachability from the application, purge exactness with two mutations, and the set's own invariants) — plus **+10** in `tests/test_attendance.py` (34→44), **+2** in `tests/test_site_picker.py` (30→32) and **+2** parametrised cases in `tests/test_import_directions.py` (292→294: `projectview → attendance` and `projectview → settings`). 20+16+10+2+2 = 50. <br><br>⚠ **NO GOLDEN MOVED.** All eleven assertions in `tests/test_print_golden.py` pass unchanged, as do `test_nav_reachability.py` and `test_page_chrome.py`. Nothing in this pass touches a printed sheet or the nav. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE, and that was VERIFIED rather than assumed** — regenerated with `python tools/dump_access_matrix.py` and diffed: no change. **7 roles · 79 permissions · 114 classified endpoints**, all three unchanged. **No permission was minted and no route was added**, so `tools/reconcile_role_permissions.py` had nothing to reconcile. The Site Labour panel reuses the existing `project.view` grant for the page and `attendance.view` for the panel. <br><br>⚠ **Two existing tests were rewritten and NEITHER was weakened or deleted**, each keeping its old assertion **verbatim** in a comment above it: `test_nothing_imports_the_attendance_module` → `test_only_projectview_imports_the_attendance_module`, an **allowlist of exactly one**, which is stricter than the blacklist it replaced; and `test_the_project_page_still_shows_no_labour_cost` → `test_the_project_page_builds_no_margin_total_or_net`, whose `"no cost total"` assertion is kept **unchanged** and whose `"attendance" not in src` assertion is the clause the override block lifts. A third, `tests/test_site_picker.py::test_both_registers_report_the_unmapped_string`, kept both its surviving assertions and had its `SITE NOT MAPPED` line replaced — ⚠ **it was passing while the page was visibly broken**, which is why it was rewritten rather than left. <br><br>⚠ **Every new guard is mutation-proved and the proofs are TESTS rather than notes:** the site-cell separation (put the old chip back and the guard goes red), the orphan refusal (remove the dependants and the same purge proceeds), purge exactness in **both** directions (one record too many, one too few), and the B4 withholding guard (an Owner must see the figure the three walled-off roles must not). |
...
`before_request` gate that refuses **any** of the **114** endpoints not classified in
...
| **C6** | project profit and loss | **BLOCKED** | No project P&L exists, and the project page **refuses to be one by design**: *"no revenue total, no cost total, no margin, no profit, no net, no balance … it does not do the subtraction"* [projectview.py:21-26](projectview.py#L21-L26) &mdash; ⚠ **that docstring moved from lines 11&ndash;15 on 30 August 2026 and its wording is UNCHANGED**; route `GET,POST /projects/view/<id>` [projectview.py:289](projectview.py#L289). ⚠ **The page now carries a SITE LABOUR section (§4c row 19) and it is not a P&L**: it lists attendance markings booked at the project's **site**, adds its own one column up the way five panels already did, and states on its face that the rows are not tagged to the project and that the data cannot say which project a day's labour belongs to. **No margin, no project total, no net, and no choice between the two labour bases.** The only margin arithmetic in the app is `purchase.job_cost()` [purchase.py:280](purchase.py#L280), which is scoped to a **quotation**, not a project, and ignores recorded charges entirely. Blocked on the labour-cost authority decision; see §5. | The decision first. Then: planned margin from the BOQ against actual cost from POs and charges, and an explicit reversal of `projectview.py`'s standing prohibition — that docstring is a deliberate guard and must be amended, not ignored. | ☐ |
...
address-book guards, and **I** the Site Labour section, the live data cleanup
and the demo scenario tool, all four 30 August 2026.
