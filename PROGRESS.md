...
| | |
|---|---|
| **Date** | 8–9 September 2026, **SIXTEENTH pass** *(a **VERIFICATION** pass. It moves **no bar**, builds **no CC-2 item**, and its purpose is to convert two claims of the form “it was written” into claims of the form “it was run”. `tools/backfill_measurement_grid.py` was **run with `--write`** and `tools/e2e_chain.py` was **run against the live app for the first time** — both shipped unrun by the fifteenth pass, which said so. Two defects fixed: **ABOUT.md §7 gap 31**, the false over-claim display, and the **`SECRET_KEY` startup warning** (CC-2, security items promoted by 3B). Authorised by the **twenty-fourth `CLIENT_CHANGES.md` §0 block**, 8 September 2026, which **re-takes** the three-account identity authorisation rather than citing the unexercised one. ⚠ **The brief for this pass carried ONE wrong premise** — that `SECRET_KEY` still defaults to a demo value; it does not and has not since 26 August 2026. Correcting it rather than propagating it is why Task D is a warning and not a reinstated fallback. **3A stays 6 of 6, 3B stays 8 of 8, 3C stays 5 of 6.** ⚠ **C6 is STILL BLOCKED on Open question 4 and nothing here touches it.**)* |
| **Machine** | home laptop — `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `3ce6eb4` — *“SECRET_KEY: the fallback now says so, loudly, at startup — and still does not refuse to boot”*, the fourth of the five commits of this pass — **in the order the brief required**: `6265353` the §0 block **alone and first**, `c4f672d` gap 31, `4cf2f03` the driver, `3ce6eb4` the `SECRET_KEY` warning, and this edition fifth. One commit per item, as required. |
| **vs `origin/antigravity-dev`** | **measured at the start of this pass with `git rev-list --left-right --count`: 0 ahead, 0 behind** — `origin/antigravity-dev` and `HEAD` were the same commit, `bcd8e22`. ⚠ **This does NOT match the fifteenth pass's recorded “84 ahead, unpushed”**, so the branch was pushed between the two passes by somebody other than an agent; that is recorded, not explained. The pass added **4** commits and this edition makes **5**. **Nothing was pushed by this pass.** |
| **Dirty files** | **0. `git status --porcelain` was empty at the start of this pass**, and the branch was verified as `antigravity-dev` before anything was written. Two backup **PAIRS** were taken — `backups/samruddhi_qms-20260908-224757-pre-verify-0908.sql` (**685,034 bytes**) before any write, and `backups/samruddhi_qms-20260908-230042-pre-e2e.sql` (**686,948 bytes**) immediately before the driver's first write run — each with its `-attachments.zip` (**22 bytes, 0 files**). ⚠⚠ **LIVE RECORDS WERE MUTATED BY THIS PASS, deliberately and under authorisation, and this is the first pass of which that is true in this way.** (1) The grid backfill marked **2 measurement sheets** `legacy-linear` and wrote one `settings` migration record — **159 → 160 records, permanent and intended**. (2) The driver created **three throwaway accounts** and, across six runs, a project / BOQ / measurement / challan / four RA bills / one receipt each — **all removed**. A closing dump, `samruddhi_qms-20260909-001002-post-verify.sql`, was diffed against the pre-e2e pair **per collection**: every table identical, the **only** delta `users: 7 → 4`, which is exactly the three throwaway accounts going away. **0 orphans, 0 throwaway accounts, `roles` untouched at 7.** ⚠ The scratch space was the session temp directory, outside the repository tree, and nothing committed references it; the generated credentials never reached a committed file or a log line. |
| **Test figure** | **measured at the end of the pass: 2,235 passed, 2 skipped.** Configuration: global `C:\Program Files\Python310` (CPython 3.10.11), **no `.venv`**, openpyxl **absent**, both client workbooks **absent**. The `.venv` configuration (CPython 3.10.11, **openpyxl 3.1.5 present**, workbooks absent) reports **2,236 passed, 4 skipped** against this commit. **Both were measured in this pass, neither was quoted.** The baselines were **re-measured at the start of this pass rather than taken from the brief or the previous edition** — **2,204 / 2** and **2,205 / 4** — and both matched what the fifteenth pass recorded. <br><br>**+31 passed and +0 skipped in both configurations, reconciling exactly** and **measured per file**: `tests/test_e2e_chain_helpers.py` (**63 → 84, +21**), `tests/test_auth.py` (**38 → 44, +6**) and `tests/test_dashboard_boq_ra.py` (**47 → 51, +4**). 21 + 6 + 4 = **31**. The row-2/row-3 relationship holds unchanged: 2,235 + 1 passed and + 2 skipped = 2,236 / 4. <br><br>⚠ **NO GOLDEN MOVED.** All sixteen digests in `tests/test_print_golden.py` pass unchanged; `/boq/view` is not golden-pinned and the gap 31 fix touched nothing that is. <br><br>⚠ **`docs/ACCESS_MATRIX.md` DID NOT MOVE** — **no route was added and no permission was minted**, so there was nothing to regenerate; `tools/reconcile_role_permissions.py` was run anyway and reported nothing to do. <br><br>⚠ **One existing assertion was repointed and NOT weakened**, keeping its old form **verbatim** in a comment above it: `tools/e2e_chain.py`'s assertion 7 was a `known_bad` asserting that `/boq/view` **disagreed** with the claim total by exactly the GST. It was written to flip the day gap 31 was fixed. It was fixed, in this pass, so the line is now an ordinary assertion that the two **agree** — the tripwire doing its job, not a test bent to fit. <br><br>⚠ **Every new guard is mutation-proved and the proofs were RUN. Sixteen mutations, fifteen caught and one discarded as a no-op.** Two on gap 31 (chip → `grand_total` fails four tests, chip → `net_payable` fails three — the second matters because `net_payable` is *also* tax-exclusive and still wrong), three on the `SECRET_KEY` warning, and eleven on the driver's helpers. ⚠ **The discarded one is recorded rather than hidden**: a first attempt at mutating `progress_rows()` changed a regex in a way the fixture could not distinguish, so it proved nothing; it was replaced by two that do bite (a greedy body that collapses the rows, and a name read from the page instead of the row). ⚠ **One of this pass's own new tests was wrong when written and its own run found it** — the `data-line-id` regression guard did a string search over `e2e_chain.py` and failed on the comment **explaining** the fix; it now reads the AST, which is `test_auth.py`'s technique for the demo secret key and for the same reason. |
