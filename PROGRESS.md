...
| | |
|---|---|
| **Date** | 25 September 2026, **the PRE-HANDOVER pass** *(the owner's own decision, CLIENT_CHANGES.md §0 **thirty-first** block, dated today. The client's production box will be deployed from whatever this pass pushes, and these are the three things that are wrong on a client's server and right on a demo one. **A** — demo data is OFF by default: `SAMRUDDHI_DEMO_DATA` gates the four DEMO seeders, the spec library / charge heads / measurement grid columns are GENUINE DEFAULTS and are not gated, and a delete now outlives a restart (§7 gap 35 closed). **B** — a settable starting-number FLOOR per series: `next = max(existing max in the scope + 1, floor)`, an FY-reset series' floor stored WITH its financial year so Rule 46(b)'s April reset survives, in a new leaf `series.py` (§7 gap 36 closed). **C** — the suite no longer reads the checkout's `.env`, so running it on the production box is a usable post-deploy check. ⚠ **Not a CC-2 item, no charge, and NO BAR MOVES** — the board is **19 of 20 BUILT · 1 BLOCKED**, as it was. ⚠ `quotation.py` and `product.py` are **NOT unfrozen** and were **not edited**: no fifth carve-out was opened, the quotation series deliberately takes no floor, and the demo product seeder is disarmed from outside by `app.disarm_frozen_demo_seeder()` — the trick the 11 September catalogue hide used. ⚠ **Gap 32's collision fix was authorised in advance and turned out NOT to be needed**: TI / RI / MI were measured against each other and produce **zero collisions** across 205 ordinals, both branches of `pipeline.fy_ref()`'s cap and every cross-year pair, so the three series were **not** unified and `ra.py` still does not import `invoice.py`. Gap 32 stays **OPEN** and the live record it reports was **not touched**.)* <br><br>*Previously: 23 September 2026, **the universal delete pass** *(the owner's own decision, CLIENT_CHANGES.md §0 **thirtieth** block, dated today: every document type becomes deletable, deletion becomes **Owner-only** through `auth.OWNER_ONLY`, every confirmation page **names the transitive closure before the button** through `cascade.impact_of()`, and a **Tax Invoice is never deleted** — only cancelled, because a GST series has to stay consecutive. Five endpoints minted: `/boq/delete`, `/quotation/delete`, `/proforma/delete`, `/purchase/delete`, `/invoice/cancel`. ⚠ Carries a **THIRD narrow unfreeze of `quotation.py`** for exactly one function, `delete_quotation()`; `product.py` was **not** edited. Not a CC-2 item, no charge, **no bar moves** — the board is **19 of 20 BUILT · 1 BLOCKED**, as it was.)* <br><br>*Previously: 12 September 2026, **the four-change pass** *(the owner's own decisions, CLIENT_CHANGES.md §0 twenty-eighth block, dated today and first: **1** the quotation source FOLLOWS the catalogue switch — the product picker restored from `1d7725a` byte for byte, the spec picker moved into the leaf `specpick.py`; **2** SUPPLY ONLY — no installation leg on a quotation, §7 gap 37 closed; **3** §7 gap 38 closed — a from-scratch PO of extra lines alone is an order, always, and the item rows follow the switch; **4** the APPROVAL LADDER SWITCHED OFF in code — `approval.LADDER_ON = False`, read through `ladder_on()`, every reader following it, the measurement cap still binding, every record raised while off stamped at create. Changes 1–3 are not CC-2 items, no charge, and move no bar. Change 4: 3B.05–3B.07 are deferred under MG/SF/2026-06 as sequencing, the ladder returns after a trial period, measurement approval goes off with it by the owner's decision; **built code stays, B6, B7 and C2 stay BUILT, no bar moves.** The board is **19 of 20 BUILT · 1 BLOCKED**, as it was.)* |
| **Machine** | home laptop &mdash; `c:\Users\manas\OneDrive\Desktop\Demo_Works` |
| **Branch** | `antigravity-dev` |
| **HEAD** | `9d826f8` — *“Cut the test suite loose from the checkout's .env”*, the fourth of the five commits of this pass, **one per item and in the order the brief required**: `f7c48a5` the §0 block **alone and first**, `f3419c9` item A (demo data off), `4d46e4b` item B (the starting-number floors, with the one re-baselined page golden **in the same commit as the change that moved it**), `9d826f8` item C (the `.env` isolation), and this edition fifth. Suite green in both configurations before each. The pass **started at** `e79a133`. ⚠ **The Step 0 gate did not match its expectation and the discrepancy was resolved rather than assumed**: HEAD was `1e8c1b1`, not the `e79a133` the brief named — measured **0 ahead / 2 behind** with `git rev-list --left-right --count`, which is the *behind and not ahead* branch of the gate, so `git pull --ff-only` was run and HEAD became exactly `e79a133`. Previously `a89ca39`. |
| **vs `origin/antigravity-dev`** | **measured at the start of this pass with `git fetch` then `git rev-list --left-right --count`: 0 ahead, 2 behind.** The brief expected 0/0 at `e79a133`; the checkout was two commits behind at `1e8c1b1`. Behind-and-not-ahead is the branch of the gate that says pull, so `git pull --ff-only` was run and the new HEAD reported before anything else was done. **Pushed at the end of this pass**, on the brief's own instruction, both configurations green. |
| **Dirty files** | **0. `git status --porcelain` was empty at the start of this pass and listed no untracked file**, before and after the fast-forward pull; the branch was verified as `antigravity-dev` on the home laptop (`C:\Users\manas\OneDrive\Desktop\Demo_Works`) before anything was written. One backup **PAIR** was taken before any write — `backups/samruddhi_qms-20260925-133032-pre-handover-pass.sql` (**694,242 bytes**) and `…-attachments.zip` (**22 bytes, 0 files**). ⚠ **The local `.env` was temporarily replaced by a production-shaped one to MEASURE item C, and restored**; it is gitignored and is not part of any commit. ⚠ **Nothing in this pass writes to the database.** The demo flag stops seeders seeding and deletes nothing; a floor is a settings record nobody has set; no live record is read, converted or rewritten. |
| **Test figure** | **measured at the end of the pass: 2,989 passed, 4 skipped.** *(25 September 2026 — the PRE-HANDOVER pass, the owner's own decision under CLIENT_CHANGES.md §0's **thirty-first** block: **A** demo data OFF by default (§7 gap 35 closed), **B** a settable starting-number FLOOR per statutory series (§7 gap 36 closed), **C** the test suite cut loose from the checkout's `.env`. **+142** over the 2,847 recorded below: **+28** in `tests/test_demo_data_flag.py` (new), **+19** in `tests/test_print_golden.py` (the blank-identity sweep over all nine print routes, 2 of them skipped by design — a challan and a measurement sheet carry no statutory identity block), **+59** in `tests/test_series_floors.py` (new), **+31** rows in `tests/test_import_directions.py` for the new leaf `series.py`, **+7** in `tests/test_env_isolation.py` (new). ⚠ **ONE GOLDEN MOVED and NO PRINT GOLDEN DID.** `tests/test_page_golden.py`'s `settings` row was re-baselined in item B's own commit — **`main` alone**, +5,560 bytes, **27 lines added and 0 removed**, measured by rendering the page twice with and without the new section before the baseline was touched. `head`, `shell` and `chrome` are byte-identical, every existing section of the form is byte-identical, and all nine printed documents are unchanged — which is what moving the nine minters' scans into `series.py` had to prove. A gap that can only be closed by putting a control on a page cannot be closed without moving that page's digest. ⚠ **`docs/ACCESS_MATRIX.md` did NOT move** — no permission, role or route classification changed: the floors sit under `/settings`' existing permission and **no permission was minted**. ⚠ **`product.py` and `quotation.py` were NOT edited and are NOT unfrozen** — no fifth carve-out; the demo product seeder is disarmed from `app.disarm_frozen_demo_seeder()` and the quotation series deliberately takes no floor. ⚠ **No bar moves and no CC-2 item changes state** — the board is **19 of 20 BUILT · 1 BLOCKED**, as it was; this is go-live hardening, not scope.)* <br><br>*Previously: **2,847 passed, 2 skipped.** *(23 September 2026 — the universal delete rollout. **+42** over the 2,805 recorded below: **+28** in `tests/test_delete_rollout.py` (new — the five per-route GET-does-not-mutate tests the standing rule requires, the BOQ cascade, the Tax Invoice hard stop from two hops up, the cancel flow, the two dangling-reference repairs and the two master-data guards), **+9** in `tests/test_cascade.py` and **+5** in `tests/test_owner_only_delete.py` (both written 22 September and uncommitted until now). ⚠ **Four existing tests were rewritten and every one got STRICTER, not looser** — the frozen-module check now runs **per line** with decorator-aware spans; `test_the_tax_invoice_has_no_edit_route_to_gate` now also asserts the no-delete prohibition outright; the attendance one-writer rule now checks its exempted readers for **writes**; and the C4 scope check exempts one named function and then proves it read-only. ⚠ **`docs/ACCESS_MATRIX.md` MOVED and was regenerated** — **7 roles · 84 permissions · 132 classified endpoints**, five permissions minted (`boq.delete`, `quotation.delete`, `proforma.delete`, `purchase.delete`, `invoice.cancel`). ⚠ **Four goldens moved and every one moved `head` ALONE**, with `main` / `doc-box` byte-identical: the tax invoice document and register (+1,698 / +1,575 bytes of `.lc-*` overprint rules), the receipts register and the `/po/create` picker (+462 bytes of `cascade.CASCADE_STYLES`). No printed document a customer receives changed; only what those pages are capable of drawing did.)* <br><br>*Previously: **2,805 passed, 2 skipped.** *(22 September 2026 — the identity pages stop offering what they then refuse: signed in as a Director, `/users/create` drew the **Owner** role on the same checkbox list as Sales Manager and then refused the save, ABOUT.md §2g and §5 `/users`; **+15** in `tests/test_identity_offers.py` (new). Four controls now follow the two guards that were already there — `_may_grant()` per role box, `_may_administer()` per register row, `can_reach()` on the Roles button — and **no guard was changed or weakened**; the forged-POST case that proves it predates this pass. A UX defect fix on built Phase 3B work: **no bar moved and no CC-2 item changed state**; the `.venv` configuration reads **2,806 / 4**. It read **2,790 / 2** and **2,791 / 4** at the start of the pass, re-measured and matching. ⚠ **`docs/ACCESS_MATRIX.md` did NOT move** — no permission, role or route classification changed, and `tests/test_access_matrix_doc.py` passes against the committed grid unchanged. **One page golden moved**: `/users` in `tests/test_page_golden.py`, **`head` alone** (+262 bytes of `AUTH_ADMIN_STYLES`), with `main` byte-identical — that page is hashed as an **Owner**, who may administer every account, so nothing an Owner sees changed at all.)* *(20 September 2026 — the BOQ sheet fits a portrait page: a phone's print service picks the paper, and Chrome for Android scaled the landscape page box onto it, ABOUT.md §5 `/boq`; **+4** in `tests/test_boq.py` (*The sheet on a portrait page*). A defect fix on an existing document: **no bar moved and no CC-2 item changed state**; the `.venv` configuration reads **2,791 / 4**. It read **2,786 / 2** and **2,787 / 4** at the start of the pass, re-measured — ⚠ **94 above the 2,692 / 2,693 recorded here**, because the seven commits `0c493e1` … `5f4ddc0` of 14–15 September (the print-route pins, the page goldens, the chrome extraction, the sidebar, the measurement-grid fix and the deployment runbook) added 94 across 17 test files and moved no figure in any document.)* *(14 September 2026, third change — the placeholder mark narrowed to the server fill — *"if I have entered price it is not a placeholder"*, ABOUT.md §5 `/purchase`; **+5** in `tests/test_po_extra_lines.py` §6 (52 → 57), four rewritten. The owner's own ruling, recorded in CLIENT_CHANGES.md §0's twenty-ninth block: **no bar moved and no CC-2 item changed state**; the `.venv` configuration reads **2,693 / 4**. It read **2,687 / 2** and **2,688 / 4** before, re-measured and matching.)* *(14 September 2026, second change — the two PO repeaters on the upstream forms and the reprice form's Amount column, ABOUT.md §5 `/purchase`; **+27** in `tests/test_po_upstream_repeaters.py` (new — 20 for the repeaters and the Amount column, 7 for the extra-part HSN box, which also moved the PO print golden by +66 stylesheet bytes). The owner's own decision, CLIENT_CHANGES.md §0 twenty-ninth block: **no bar moved and no CC-2 item changed state**; the `.venv` configuration reads **2,688 / 4**. It read **2,660 / 2** and **2,661 / 4** before, re-measured and matching.)* *(14 September 2026 — the BOQ editor quantifies a new line at 1, ABOUT.md §5 `/boq`; **+6** in `tests/test_editor_nav.py` (27 → 33). UX only: **no bar moved and no CC-2 item changed state**; the `.venv` configuration reads **2,661 / 4**. It read **2,654 / 2** and **2,655 / 4** before, re-measured and matching.)* *(13 September 2026 — the BOQ line panel rearranged into four bands and a fold, ABOUT.md §5 `/boq`. **UX only: +4 in `tests/test_editor_nav.py` (23 → 27), no Phase 3 item moved**, so the rest of this header still describes the 12 September pass, whose end figures were **2,650 / 2** and **2,651 / 4**.)* Configuration: **openpyxl absent**, both client workbooks **absent** — on the machine this pass ran on that is the `.venv` itself; see the note below. The `.venv` configuration (**openpyxl 3.1.5 present**, workbooks absent) reports **2,990 passed, 6 skipped** against this commit. ⚠ **Both figures were re-measured in this pass on a DIFFERENT MACHINE from the one row 2 names** — `C:\Users\infra\dev\Demo_Works`, **CPython 3.14.6**, and there is no global CPython 3.10.11 on it. The two rows are therefore *openpyxl absent* and *openpyxl present* **in one interpreter** rather than two interpreters. The +42 delta reconciles in both, and the absent/present relationship is unchanged from every previous pass: without openpyxl `tests/test_fixtures.py` skips at **module** level (one skip, three tests uncollected); with it the three collect and skip individually on the missing workbooks. That is the whole of the 1-passed / 2-skipped difference, and it is why the `.venv` figure is the larger one. **Both were measured in this pass, neither was quoted**, and both were measured after every one of the four change commits. The baselines were **re-measured at the start of this pass — 2,432 / 2 and 2,433 / 4 — and matched the documents exactly.** <br><br>**+218 passed and +0 skipped in both configurations, reconciling exactly, per change and per file**: change 1 `tests/test_quotation_switch.py` (**18**, new — the OFF-state golden captured from a `git worktree` of `1d7725a`, its control, both modes, the flip refusals, quotations from either mode raising a PI and a TI under either switch state, the eight seams, the leaf), `tests/test_escaping.py` (**+2**, the script-embed test parametrised over both embeds — product in OFF, library in ON) and `tests/test_import_directions.py` (**+12** rows for `specpick.py`); change 2 `tests/test_quotation_spec_picker.py` (**+7**, 48 → 55 — the install-leg tests retargeted, not deleted, each keeping its old name in its docstring); change 3 `tests/test_po_gap38.py` (**28**, new, most parametrised over both switch states); change 4 `tests/test_approvals_off.py` (**148**, new) and `tests/test_approval_grandfather.py` (**+3**). 18+2+12+7+28+148+3 = **218**. The row-2/row-3 relationship holds unchanged: 2,650 + 1 passed and + 2 skipped = 2,651 / 4. <br><br>⚠ **NO PRINT GOLDEN MOVED.** All sixteen digests in `tests/test_print_golden.py` pass unchanged after every commit. **The OFF-state quotation golden MATCHES**: `/quotation/create` with the switch emptied renders `b467ca97ab34d077` / 87,068 bytes, captured from `1d7725a`'s own code with the date, the catalogue, the address book and the counter fixed — ten blocks, all identical. <br><br>⚠ **`docs/ACCESS_MATRIX.md` MOVED and was regenerated in change 4's commit** — no permission was minted and no route was added (**7 roles · 79 permissions · 132 classified endpoints**, unchanged), but the five `*.approve` rows carry a new **`⊗` switched-off** mark, distinct from `⊘`, and the appendix names the twelve approval endpoints and the constant. <br><br>⚠ **Existing ladder tests were NOT rewritten into OFF checks.** Nine modules run with the ladder ON through the new `conftest.ladder_on` fixture (`test_approval`, `test_approval_b7`, `test_attachments`, `test_measurement`, `test_c1_order_of_working`, `test_merged_ra`, `test_dashboard_boq_ra`, `test_escaping`, `test_entity_fallbacks`) plus one B2 test in `test_auth.py`; every one keeps proving the ladder for the day it returns. <br><br>⚠ **Mutation proofs RUN: the cap, the `{}` path and the stamp** — `accepted()` replaced by the literal approved test lets a claim above the saved sheet through; `feeds_ceiling()` replaced the same way makes the ceiling read `{}` and `ra.py` fall back to the BOQ; the stamp removed makes a record raised now gated the day the ladder returns. All three caught. <br><br>**Records per document type by approval state, counted from the Step 0 dump (the file, not the live database):** charges 5 — all pending, grandfathered; RA bills 7 — all pending, grandfathered, one with one rung already climbed; merged RA 0; tax invoices 1 — pending, grandfathered; purchase orders 3 — 2 pending grandfathered, **1 pending with one rung climbed and NOT grandfathered** (`SF/PO/26-27/0003`, Draft); measurement sheets 2 — both pending, neither grandfathered (`SF/MS/26-27/0001`, `0002`). **Nothing is approved or rejected anywhere.** The **three** records that were PENDING before the switch went off carry no mark and **return to pending when it comes back on**: `SF/PO/26-27/0003`, `SF/MS/26-27/0001`, `SF/MS/26-27/0002`. |
...
`before_request` gate that refuses **any** of the **132** endpoints not classified in
...
⚠ **`rate_is_assumed` means *the server put this figure here* — narrowed on
14 September 2026 by the owner: *"if I have entered price it is not a
placeholder."*** It is true when, and only when, the rate box was posted blank
and the server filled it from `po_parts.py`; a figure in the box on submit is
the operator's price, the seeded figure included. Until that day the rule was
the wider one — any rate *equal* to the seed was marked however it got there,
as the safe direction — and the owner's own order that morning (`150mm flange`
at ₹1,350, `16 sq mm wire` at ₹175, both entered) was chipped for it. So the
form now **offers** the seed as ghost text and `data-seed` and never writes it
into the box, and a stored placeholder line reopens on the reprice form with a
blank box, so a resave keeps it a placeholder. Still derived on the server,
still no hidden field; the mark is set at the fill site and `_extra_assumed()`
is deleted. **+5** in `tests/test_po_extra_lines.py` §6, four rewritten; no
bar moves and no stored record is rewritten.
...
1. ~~**No repeater on `/purchase/from-boq/<boq_id>` or
   ...
   made, made again.**~~ ✅ **REVERSED 14 September 2026, by the owner's
   decision** — both repeaters, A3's charges included, are on both upstream
   forms now, drawn by the same two section helpers and parsed by the same two
   parsers, the lists entering through `_totals_of()` as everywhere else. The
   owner raised an order from a draft and found both sections missing from the
   form that goes to the vendor while the reprice form beside it carried both.
   The schedule-line rule did not move. Both routes still write
   `extra_lines: []` when none is typed. ABOUT.md §5 `/purchase`;
   `tests/test_po_upstream_repeaters.py` (20). **The same pass closed a
   defect on the reprice form**: its `recalc()` was an empty stub, so the
   Amount column there never filled — the JavaScript is one helper now,
   `_po_form_script_html()`, on all four forms. **And a second defect**: the
   printed order flagged every extra part `[add HSN]` — to the vendor — while
   no form offered an HSN box for one. The repeater carries `extra_hsn` now,
   stored as `hsn`, printed where typed and chipped where blank; `po_parts.py`
   seeds none. The PO golden moved **+66 bytes, `head` only, all stylesheet**.
...
30 August 2026 — **K** the attachments work of 2 September 2026, **L** the
single-leg statutory serial of 3 September 2026, and **M** the dashboard's
BOQ/RA visual cues of **5 September 2026**.
...
| 28 | **The dashboard's BOQ/RA visual cues** — an open-BOQ tile, an RAs-pending-approval tile, a claimed-to-date tile carrying its own denominator, a permission-filtered BOQ/RA activity feed, and per-project claimed-against-approved bars, all on the existing `/` | CC-2's **untagged lines 1 and 2** ("visual dashboard"), and ABOUT.md §7 gaps 24, 27 and 31 | **M** | ⚠ **CC-2 CONTAINS NO TAGGED ITEM FOR THIS, AND THE UNTAGGED ONE IS A QUESTION RATHER THAN A SPECIFICATION.** *"Director — visual dashboard"* and *"Operation Head — visual dashboard"* are lines 1 and 2 of CC-2's *"Stated by the client, NOT in MG/SF/2026-02"* section, which says of them that they *"need a client ruling before they can be priced at all"*, that whether the phrase meant the existing dashboard restricted by role or a new analytics view is *"not answerable from any document we hold"*, and — in terms — *"do not take the cheaper reading because it is cheaper."* ⚠ **This pass does not answer that question and must not be recorded as having answered it.** It adds five surfaces to the dashboard that already existed; it takes no view on what the client meant, restricts nothing by role that was not already restricted, and builds no analytics view. The lines stay untagged, unpriced and open. ⚠ **The brief for this pass cited a *"Visual Dashboard is free, not chargeable"* ruling of 19/24 August 2026. No such ruling exists** — it was searched for in all five documents before the §0 block was written — and recording one would have asserted a client decision nobody has taken. The §0 block of 5 September 2026 records that instead. ⚠ **Half of it is a defect fix and only half.** Every amount on the band is tax-exclusive, which is the second of the two options §7 **gap 31** sets out — the bug that made `SF/BOQ/26-27/0006` read ~18% over-claimed when it was claimed to exactly 100%. **`/boq/view` is untouched and gap 31 stays open there**, so this demonstrates the fix rather than being it. ⚠ **It NARROWS gap 27 without closing it, and the line drawn is ours.** The activity feed names a document, its project and its amount, so it is filtered per record; the aggregate tiles beside it are not, and that inconsistency is intended — naming a document to somebody who cannot open it is a bigger disclosure than counting how many exist. ⚠ **And it is LIMITED BY gap 24**: the filter is endpoint-level because this application has no object-level check to reuse, so `auth.can_reach()` *is* the gate rather than an approximation of one. |
| 29 | **`tools/e2e_chain.py`** — a standalone end-to-end HTTP chain driver: an empty project through to a fully claimed BOQ, over the wire, with twelve tally assertions and a reverse-dependency-order teardown | nothing | **N** | ⚠ **NO CC-2 ITEM AND NO MG/SF/2026-01 ITEM.** It exists because the suite runs with `DB_ENABLED=false`, a cleared `STORE` and a tmpdir, so **2,081 green tests say nothing about real MySQL, real sessions, real permission checks or real form posts** — and three defects in this project's history were invisible to the suite and visible only in a browser (`/measurement/` unreachable on live roles, the attendance render defect, the `/boq/view` unit mismatch). ⚠ **Stdlib only, deliberately**: `requests` is on the global interpreter and **absent from the `.venv`**, so a driver built on it would run in one of the two configurations this project is tested in. ⚠ **Forms are GOT, parsed whole and posted back**, never hand-assembled — that survives a CSRF token without knowing one exists, and a field the form gained since travels through instead of being dropped to its default. ⚠ **THREE accounts, not the two the brief specified**, and this was measured: the creator guard stops A approving what A raised, and rule 5 (*one user, one rung*) stops one approver climbing both rungs of the two-rung RA ladder. ⚠⚠ **IT HAS NOT BEEN RUN AGAINST THE LIVE DATABASE.** Its 63 offline helper tests pass and all seven mutations are caught; the app was started, the login refusal path and the closed-app redirect were exercised. **Every one of the twelve tally assertions in §2.7 is therefore UNEXERCISED**, and the pass report says so in its first line rather than at the end. |
| 30 | **The JOINT MEASUREMENT SHEET** — C2's printed document redrawn as a location × diameter matrix on the client's own workbook layout: landscape, the shared letterhead, five header rows, twelve unit-bearing numeric columns under spanning `SUPPORTS` and `SPRINKLER` heads, seventeen seeded locations, a TOTAL row, and a **two-party countersignature** | **C2** (its printed half only — the item does not move) | **N** | ⚠⚠ **THE BRIEF INSTRUCTED THAT MEASUREMENT STOP CAPPING THE RA CLAIM, AND THAT INSTRUCTION WAS WITHDRAWN.** It said not to *add* a cap or a per-line `line_id` link. **Both already existed and shipped**: `ra.overclaims()` replaces the BOQ ceiling with `MS.approved_qty_by_line()` for the installation leg, and that reads `items[].line_id`. The grid carries no `line_id`, so building it *instead of* `items` would have deleted two live guards — **silently, through the `{}`-means-grandfather path that `ra.py` reads as "this project predates measurement"** — and contradicted **the only sentence CC-2 actually specifies about measurement**. Manas ruled on 6 September 2026 that the cap stays; a joint sheet carries **both** halves and `ra.py` is not modified. ⚠ **CC-2 SAYS NOTHING ABOUT WHETHER A MEASUREMENT PRINTS OR WHAT IT LOOKS LIKE** — the fifth 29 August 2026 block already recorded that as ours, and this pass **replaces one unspecced layout with another**. A transcribed layout is evidence of what they use, not an instruction we were given. ⚠ **Deliberate departures from their paper, recorded so nobody reverts them**: every numeric column is totalled over every row (**their `25 NB` and `SUPPORTS (MSA kgs)` have no total at all, and an unlabelled column sums rows 9–35 where every other sums 9–25**), the unlabelled column is dropped, and the ten blank ruled filler rows are **not printed** — a blank row under a countersignature invites post-signature insertion, which is the delivery challan's argument and is **stronger here because the customer signs too**. ⚠ **That filler-row call still needs Yogesh's confirmation**, exactly as the DC one does. ⚠ **Columns are DATA** — twelve definitions in `/settings` on the charge-heads pattern, **snapshotted onto each sheet at create** so a settings edit cannot restate a sheet somebody has signed; that invariant has already shipped as a defect once in this repo. ⚠ **SITE is inherited and has no input at all**: three spellings of one city are already live because site was free text in three places, and a fourth box would repeat the 30 August cleanup. ⚠ **Both live sheets take the LEGACY branch** — neither carries a location or a diameter as structured data, so migrating would mean **inventing** it. |

| 31 | **Gap 31 closed** — `/boq/view`'s RA chips render `claim_subtotal`, so the chip strip and the *Total Basic Value* tile are the same kind of number and can legitimately be added up | nothing — **ABOUT.md §7, not a CC-2 item** | **N** | ⚠ **A DISPLAY DEFECT, NOT A BILLING ONE. No RA, BOQ or claim record was touched and none needed to be.** The panel put a tax-EXCLUSIVE tile labelled *“taxes extra”* inches above tax-INCLUSIVE `grand_total` chips labelled nothing, and the layout invites exactly one comparison: add the strip up, read it against the tile. On any BOQ claimed to completion that comparison is wrong by the GST. ⚠ **It had already cost a real investigation**: `SF/BOQ/26-27/0006` (*Work2*) was reported over-claimed on 3 September 2026 at 885 + 10,425 = **11,310** against a **9,585** schedule; the `claim_subtotal`s are 750 + 8,835 = **9,585**, the schedule exactly, and the **1,725 “overage” was the GST**. ⚠ **The two screens openly contradicted each other** — `dashboard._boq_ra()` has summed `claim_subtotal` since 5 September and always showed Work2 at 100%. The chip now uses **the same field by the same expression**, and the strip carries the unit on its face: *“claimed basic value · taxes extra”*. ⚠ **`net_payable` is NOT the alternative** and the mutation proves the tests know the difference: it is also tax-exclusive, it nets off retention, and money withheld from a claim was still claimed against the schedule. Four assertions on the live Work2 shape assert the **equality between the two screens** rather than pinning either to a literal. Verified on live data through the running app, and independently by the driver's assertion 7. |
| 32 | **`SECRET_KEY`'s fallback warns loudly at startup** — `resolve_secret_key_with_source()` reports which of the four sources answered, and `install()` prints a stderr banner when nobody supplied a key | CC-2 **“Security items promoted by this phase”**, inside **3B** | **N** | ⚠⚠ **THE BRIEF'S PREMISE FOR THIS ITEM WAS WRONG AND IS CORRECTED HERE RATHER THAN PROPAGATED.** It said `SECRET_KEY` “still defaults to a demo value” and asked for an env var with a fallback kept. **It has not defaulted to anything since 26 August 2026**: the literal `qms-demo-secret-2024` was **deleted, not demoted**, two environment variables were already read, and `test_the_demo_secret_key_is_gone_from_the_codebase` reads every root module's AST to keep it deleted. **Building the brief as written would have re-opened gap 8** — a fallback that only fires “in development” is a fallback that ships. What was genuinely missing was any signal that the third path had been taken, and that is what shipped. ⚠ **It warns; it does not refuse to start**, deliberately and on the record: failing hard on a missing variable breaks the dev flow and takes the suite with it, and a developer who cannot start the app sets the variable to anything at all — which is worse than a real random 256-bit key. ⚠ **stderr, never a page** — telling an anonymous visitor how the sessions they would like to forge are signed is the opposite of the point, and that is asserted. The suite never sees the banner because `conftest.py` sets `SECRET_KEY`; this machine's `.env` supplies one too, so the fallback was exercised by clearing the variable after `db.py`'s `load_dotenv()`. |
| 33 | **`tools/backfill_measurement_grid.py` RUN, and `tools/e2e_chain.py` RUN** — the two things the fifteenth pass shipped and recorded as unrun | **C2** (the backfill) and **C1** (the driver, exercising **C2** and the **B6**/**B7** ladder) | **N** | ⚠ **NEITHER ITEM MOVES. C1 and C2 were BUILT before this pass and are BUILT after it** — running a driver over them is not building them, in exactly the way the fifteenth pass says redrawing a sheet did not move C2. **The backfill:** 2 sheets marked `legacy-linear`, one `settings` migration record written, **idempotence proved from the actual second run's output** (`to mark: 0`, and it returns before the write branch even with `--write` given) rather than by reading the code; quantities unchanged at 10.00 and 5,865.00, verified by re-reading MySQL in a fresh process. **The driver:** six runs, three throwaway accounts, all torn down; **25 PASS, 0 FAIL, 1 KNOWN-BAD, 0 SKIP**. ⚠ **The first run found six defects in the driver and one in the application**, none of which its 63 offline tests could see — which is the argument for the driver, made by the driver. The application defect is **new §7 gap 33**: `/boq/create` cannot attach a BOQ to a project, so a schedule raised through the screen files its claims under *Unassigned*. Recorded as KNOWN-BAD with a live tripwire, **not fixed**: a project picker changes what a screen shows and is new scope. ⚠ **The driver's teardown was reporting deletions it had not made** — `/boq/delete/<id>` does not exist and the closed-app gate answers an unrouted address with a redirect, so “no error flash” read as success. Deletes are now confirmed by re-reading the record. ⚠ **A chain that issues bills and approves measurements cannot fully tear itself down over HTTP**, by design and correctly: an issued RA bill and an approved measurement sheet both refuse deletion. Those four records per run were removed deliberately at store level and the closing dump diff proves it. |
| 34 | **Gap 33 closed** — `/boq/create` renders a `project_id` selector, and `/projects/view/<id>` gains a **detach** action beside the attach that was already there | nothing — **ABOUT.md §7, and a DEFECT against MG/SF/2026-02 §3 **F.04**, not a CC-2 item** | **N** | ⚠⚠ **THE SIXTEENTH PASS CLASSIFIED THIS AS NEW SCOPE AND THAT CLASSIFICATION IS OVERRULED, not quietly reversed.** §7 gap 33 closed with *“a change to what a screen shows and is new scope”*. MG/SF/2026-02 §3 lists **F.04 — “Project grouping — BOQs grouped under a project”** among the five items *“all stated to the client as built, included in the handover, and not charged for”*. A defect is the system not doing what the quotation says it does, so this is a **§0 no-charge exemption**, not chargeable work. The §0 block of 9 September 2026 records the reclassification in terms. ⚠ **The defect was two halves of one link that never met**: the route read `?project_id=` into its prefill and read `form.get("project_id")` on POST, and the form rendered **no control of that name**, so every BOQ raised through the screen stored `project_id: ""`. The linkage on this database came entirely from `tools/backfill_projects.py`; **on a client box, which starts empty, that tool never runs**, so the sold behaviour was unreachable. ⚠ **The detach half was NOT in the gap's own “what it would take” and is the part that matters operationally**: there is **no `/boq/edit` route in this application** and the attach control offers only BOQs carrying no project, so a schedule filed against the wrong one was filed there permanently. Detach walks `ra.revision_chain()` exactly as attach does and clears only members currently on that project. ⚠ **No permission was minted and no route was added** — detach is a new *action* on the existing `projectview.view_project` endpoint, already behind the per-view `project.edit` guard, so `docs/ACCESS_MATRIX.md` did not move. ⚠ **Revision inheritance already worked** and was verified rather than rewritten. ⚠ **The import direction was declared rather than assumed**: `STORE["projects"]` is read directly, the pattern six other modules use and the one `test_import_directions.py` names in terms; `boq.py`'s module-level `import project` is **dead** (commit 119f9d8, used by nothing) and was deliberately not reached through. 17 tests, 8 mutations all caught, and the driver's assertion 8 flipped from KNOWN-BAD to a real **PASS at 100% claimed** on live data. |
| 35 | **A float-comparison audit on the over-claim and measurement ceilings** — 31 tests recording a **negative result**, §7 gap 34 | nothing — **ABOUT.md §7, not a CC-2 item** | **N** | ⚠ **THIS ROW BUILT NO FEATURE AND CHANGED NO APPLICATION CODE.** It is registered because the *tests* are new surface and because a negative result nobody wrote down gets re-investigated. **The concern was sound in shape**: quantities are floats, cumulative claimed quantity is derived by summing every prior claim and never stored, and `OVERCLAIM_TOLERANCE` is **0.0** — so a final, legitimate, exactly-to-the-BOQ claim could in principle be refused on a completed job in front of the main contractor. **It does not reproduce.** Both comparison sites already normalise — `round(cumulative - allowed, 6) > _QTY_EPSILON`, with `_QTY_EPSILON = 1e-6` — and the **worst residual measured on realistic metre, kilogram and Nos splits is `7.105427357601002e-15`**, about **1.4 × 10⁸ times** below the epsilon. ⚠ **`OVERCLAIM_TOLERANCE` was NOT raised**; the §0 block bars it in terms, because that constant encodes a business rule the client agreed to. ⚠ **The finding worth keeping is that the rounding is LOAD-BEARING**: a naive `cumulative > allowed` **would** wrongly refuse three of the eight shapes tested, so the negative result depends on an expression somebody could “simplify” away in good faith. The tests exist to stop that, and five mutations prove they bite. ⚠ **The money side was checked too and compares nothing for equality**: every tax figure is `round(..., 2)` per slab, the grand total rounds to a whole rupee, and the only float equalities on money are two `bal == 0` in `proforma.py` on an already-rounded `balance_due`, both choosing wording rather than gating anything. **No stored figure moved and no golden moved.** ⚠ **Two loose ends left open and named**: `challan.py` hardcodes the literal `1e-6` instead of a shared constant (lowest stakes — over-dispatch warns, never refuses), and `measurement.py`'s comment claims they share a figure when nothing holds them together. |
